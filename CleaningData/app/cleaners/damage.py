...
class VehicleDamage():
    ...
    def level_extraction_llm(self, text: str, user_query: str = " ",
        ...
        template = """
            ...
            - If unsure, guess the most likely level based on the available information.         
            - Returns 0 if there is not enough information.  
            - The returns are the percentage punishment that we are going to apply to the original price 

            Criteria: 
            1. **Level 1 (Minor damage)**:  
                Description:
                - Cosmetic or minor mechanical damage that does not affect the vehicles performance or safety.
    ...
    @staticmethod
    def percentage_damage(df_or) -> pd.DataFrame:

        
        df_or['Realiability'] = df_or['Realiability'].str.replace('%', '')
        df_or['Realiability'] = df_or['Realiability'].str.replace(' ', '')
        df_or['Realiability'] = df_or['Realiability'].astype(int)

        conditions = [
            (df_or['Realiability'] >= 90) & (df_or['Realiability'] <= 100),
            (df_or['Realiability'] >= 80) & (df_or['Realiability'] <= 89),
            (df_or['Realiability'] >= 60) & (df_or['Realiability'] <= 79),
            (df_or['Realiability'] >= 40) & (df_or['Realiability'] <= 59),
            (df_or['Realiability'] >= 20) & (df_or['Realiability'] <= 39),
            (df_or['Realiability'] <= 19)
        ]

        values = [0, 1, 2, 3, 4, 5]
        
        df_or['Descripcion_int'] = np.select(conditions, values, default=1)

        return df_or 
    
    @staticmethod
    def percentage_damage_single(df_or) -> pd.DataFrame:
        
        valor_str = df_or.at[0, "Realiability"]
        if isinstance(valor_str, str):
            valor_str = valor_str.replace('%', '').replace(' ', '')
        valor_dam = int(valor_str)
        
        if valor_dam >= 90 and valor_dam <= 100:
            perce_dam = 0 
        elif valor_dam >= 80 and valor_dam <= 89:
            perce_dam = 1 
        elif valor_dam >= 60 and valor_dam <= 79: 
            perce_dam = 2
        elif valor_dam >= 40 and valor_dam <= 59:
            perce_dam = 3 
        elif valor_dam >= 20 and valor_dam <= 39:
            perce_dam = 4
        elif valor_dam <= 19:
            perce_dam = 5

        else:
            raise ValueError(f'El valor {valor_dam} no es válido. Debe estar entre 0 y 100.')
